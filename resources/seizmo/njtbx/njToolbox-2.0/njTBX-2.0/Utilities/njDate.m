...
if nargin < 1, help(mfilename), return, end
    ...
    try
        if (strcmp(datestr(dateStr,1),dateStr))  % seems user didn't add the hh:mm:ss
                       dateStr = strcat(dateStr,' ',hhmmss); 
        end
        switch nargin
            case 1
                ...
                if  ((isa(dateStr, 'char') && strcmp(datestr(dateStr,dateForm),dateStr)))
                    ...
                    disp(sprintf('MATLAB:Utilities:njDate:"%s" is not a valid date string. Form "dd-mmm-yyyy HH:MM:SS" required.', dateStr));
                   return;
               end
            otherwise, error('MATLAB:Utilities:njDate:Nargin',...
